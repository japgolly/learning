...
EQ[d1, I#ap[d1]#eval]
EQ[d1, K#ap[d1]#ap[d2]#eval]
EQ[d1, S#ap[K]#ap[K]#ap[d1]#eval]
EQ[d1, I2#ap[d1]#eval]
...
EQ[T, And#ap[T]#ap[T]#eval]
EQ[F, And#ap[T]#ap[F]#eval]
EQ[F, And#ap[F]#ap[T]#eval]
EQ[F, And#ap[F]#ap[F]#eval]

type Or = λ2[λF2 { type ap[x<:λ,y<:λ] = x#ap[x]#ap[y] }]
EQ[T, Or#ap[T]#ap[T]#eval]
EQ[T, Or#ap[T]#ap[F]#eval]
EQ[T, Or#ap[F]#ap[T]#eval]
EQ[F, Or#ap[F]#ap[F]#eval]
EQ[T, Or#ap2[T,T]#eval]
EQ[T, Or#ap2[T,F]#eval]
EQ[T, Or#ap2[F,T]#eval]
EQ[F, Or#ap2[F,F]#eval]

type If = λ3[λF3 { type ap[m<:λ,a<:λ,b<:λ] = m#ap[a]#ap[b] }]
EQ[d1, If#ap[T]#ap[d1]#ap[d2]#eval]
EQ[d2, If#ap[F]#ap[d1]#ap[d2]#eval]
EQ[d1, If#ap3[T,d1,d2]#eval]
EQ[d2, If#ap3[F,d1,d2]#eval]
EQ[d1, If#ap[T]#ap2[d1,d2]#eval]
EQ[d2, If#ap[F]#ap2[d1,d2]#eval]

trait *>[x<:λ,y<:λ] extends λFinn { type eval = x#ap[y]#eval }
EQ[d1, (If *> T *> d1 *> d2)#eval]
EQ[d2, (If *> F *> d1 *> d2)#eval]
...
Eq[T, CONTAINS *> d1 *> L_d21]
Eq[T, CONTAINS *> d2 *> L_d21]
Eq[F, CONTAINS *> d3 *> L_d21]
...
trait Equal <: Bool
trait NotEqual <: Bool
trait X[x] { type equals[y] <: Bool }

trait EqTest[A] { type get[B >:A <:A] >:A <:A }
trait EqTest2[A] extends EqTest[A] { type get[B >:A <:A] = B }

implicitly[Equal =:= X[String]#equals[String]]
implicitly[NotEqual =:= X[Int]#equals[String]]